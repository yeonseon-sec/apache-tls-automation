...
# 설정 오류 시 자동 복원
# BACKUP_FILE은 apply_fix.sh 안에서 선언되므로 run_all.sh에서는 최신 백업을 찾아 복원

sudo apache2ctl configtest || {
  echo "[!] Apache config test failed. Restoring backup..."
  latest_backup=$(ls -t /etc/apache2/mods-available/ssl.conf.bak.* 2>/dev/null | head -1)
  if [[ -n "$latest_backup" ]]; then
    sudo cp "$latest_backup" "/etc/apache2/sites-available/default-ssl.conf"
    echo "[*] Restored: $latest_backup"
  else
    echo "[!] No backup found. Please restore manually."
  fi
  exit 1
}
